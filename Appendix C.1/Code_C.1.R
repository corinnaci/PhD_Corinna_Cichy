# This is code to replicate the implementation in Sections 5.5 to 5.8
...
# read input data
metrics <- read_excel(Data_C.1.xlsx)
...
gf4 <- sapply(Cons2, g4)
gf5 <- sapply(Curr1, g5)
gf6 <- sapply(Access1, g6)
gf7 <- mapply(g7, Comp1, Comp2)
gf8 <- mapply(g8, Cons1, Cons2)
gf8a <- mapply(g8a, Cons1, Cons2)
gf9 <- mapply(g9, Curr1, Access1)
...
# fitting the model using least squares and backwards elimination (Section 5.6)
lm1 <- lm(Score ~ gf1 + gf2 + gf3 + gf4+ gf5 + gf6+ gf7+ gf8 + gf8a + gf9)
lm1AIC <- step(lm1)
summary(lm1AIC) #to obtain the results shown in Section 5.8
